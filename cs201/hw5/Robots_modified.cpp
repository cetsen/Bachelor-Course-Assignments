...
int Robot::GetXCoordinate() //this member function returns the x-coordinate of the robot
{
	return xPos;
}
int Robot::GetYCoordinate() //this member function returns the y-coordinate of the robot
{
	return yPos;
}
Direction Robot::GetDirection() //this member function returns the direction of the robot
{
	return direction;
}
void Robot::TurnFace(Direction dir) //this member function gets the direction the robot will face and updates its direction
{
	direction = dir;
	theRobotWindow->Redraw(this);
}
void Robot::Resurrect() //this member function resurrects the robot when it dies 
{
	stalled = false;
	theRobotWindow->Redraw(this);
}
bool Robot::IsAlive() //this member function checks if the robot is stalled or not and updates its status
{
	if(stalled)
		return false;
	else
...
void Robot::PickAllThings() //this member function picks all things from a cell if the robot is alive and while the cell isn't empty
{
	if(IsAlive())
	{
		while(!CellEmpty())
...
void Robot::PutManyThings() //this member function puts many things on a cell if the robot is alive and the cell count isn't 10
{
	if(IsAlive())
	{
		while(!BagEmpty() && GetCellCount(xPos,yPos)!=10)
			PutThing();
		theRobotWindow->Redraw(this);
	}
}
void Robot::TurnRandom() //this member function turns a robot to a random direction
{ 
	if(IsAlive())  
	{ 
		RandGen a;
		int k=a.RandInt(1,4); //a random integer is picked from [1,4]
		if(k==1) //if the picked integer is 1, the robot turns to the north direction and moves
		{
			TurnFace(north);
			Move();
		}
		else if(k==2) //if the picked integer is 2, the robot turns to the west direction and moves
		{
			TurnFace(west);
			Move();
		}
		else if(k==3) //if the picked integer is 3, the robot turns to the east direction and moves
		{
			TurnFace(east);
			Move();
		}
		...
		{
			TurnFace(south); //if the picked integer is 4, the robot turns to the south direction and moves
			Move();
		}
	}
}
int Robot::GetBagCount() //this member function returns the number of items in the robot's bag
{
	return bag;
}
